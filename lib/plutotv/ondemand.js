(function() {
	...
	const generateM3U8 = async (config, region, bootData) => {
		...
		try {
			cache = JSON.parse(fs.readFileSync(`${outdir}/plutotv_ondemand.cache`, 'utf-8'));
		} catch (ex) {
			cache = {};
		}
		...
		for (let i = 0; i < categoriesList.categories.length; i++) {
			...
			for (let j = 0; j < items.items.length; j++) {
				...
				const tvgChno = chNo++;
				const url = bootData.servers.stitcher + path + '?' + bootData.stitcherParams;
				m3u8 += `#EXTINF:-1 tvg-id="${id}" tvg-logo="${vodItem.featuredImage.path}" tvg-chno="${tvgChno}" group=title="${catname}", ${vodItem.name}\n${url}\n\n`;
				numChannels++;

			}
			if (cacheDirty) try {
				fs.writeFileSync(`${outdir}/plutotv_ondemand.cache`, JSON.stringify(cache), 'utf-8');
			} catch (ex) {}
		}

		try {
			fs.writeFileSync(`${outdir}/plutotv_ondemand.cache`, JSON.stringify(newCache), 'utf-8');
		} catch (ex) {
