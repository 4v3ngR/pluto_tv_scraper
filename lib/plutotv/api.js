(function() {
	...
	const timelines = async (region) => {
		...
		for (let i = 0; i < channelIds.length; i += chunkSize) {
			...
			const clientTime = encodeURI(d.toISOString());
			const resp = await axios.get(`https://service-channels.clusters.pluto.tv/v2/guide/timelines?start=${clientTime}&duration=480&channelIds=${chunks.join('%2C')}`, {headers});
	...
	const generateM3U8 = async (region, group, regionalize, exclude) => {
		let m3u8 = "#EXTM3U\n\n";
		for (let i = 0; i < channelList.data.length; i++) {
			...
			if (exclude && new RegExp(exclude).test(category.name)) continue;

			const id = c.id + (regionalize && region ? '-' + region : '');
			const url = bootData.servers.stitcher + c.stitched.path + '?' + bootData.stitcherParams;
			m3u8 += `#EXTINF:-1 tvg-id="${id}" tvg-logo="${c.images[0].url}" tvg-chno="${c.number}" group-title="${catname}", ${c.name}\n${url}\n\n`;
		}
		return m3u8;
	}
