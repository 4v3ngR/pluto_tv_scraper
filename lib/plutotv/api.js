(function() {
	...
	const generateM3U8 = async (xff, region, group, regionalize) => {
		let m3u8 = "#EXTM3U\n\n";
		for (let i = 0; i < channelList.data.length; i++) {
			const c = channelList.data[i];
			const category = group === 'genre' ?
				categoryList.data.find(cat => cat.id === c.categoryIDs[0]) :
				{ name: region };
			const id = c.id + (regionalize ? '-' + region : '');
	...
	const generateXMLTV = async (xff, region, regionalize) => {
		const obj = {
			"_declaration": {
				"_attributes": {
					"version": "1.0",
